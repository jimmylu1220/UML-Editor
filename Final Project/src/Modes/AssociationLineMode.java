...
import Line_Objects.Association_Line;
...
public class AssociationLineMode extends Mode {
	...
	public void mousePressed(MouseEvent e) { //
		...
		for(int i = 0;i<draw_area.ObjectVector.size();i++) {
			...
			if(temp_obj.IsInsideGraphics(e.getPoint(),temp_obj.getLocation()) == true) {
				overlap_count ++;
				if(overlap_count > 1) { //check overlap
	...
	public void mouseReleased(MouseEvent e) {
		...
		if(StartGraphic_Index != -1 && EndGraphic_Index != -1) {
			lobj = new Association_Line(draw_area.ObjectVector.elementAt(StartGraphic_Index), draw_area.ObjectVector.elementAt(EndGraphic_Index)
